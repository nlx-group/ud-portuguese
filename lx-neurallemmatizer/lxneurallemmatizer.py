        ...
        for e in entities:
            if e["start"] > 0:
                ...
                if isinstance(word_piece, list):
                    if len(word_piece) != 1:
                        LOGGER.warning(
                            f"postprocess() got list {word_piece!r} where a string was expected"
                        )
...
class LxNeuralLemmatizerException(Exception):
    pass


class LxNeuralLemmatizer:
    ...
    def lemmatize_paragraph(self, paragraph):
        ...
        LOGGER.info(f"Lemmatizing paragraph: {plain_text}")
        ...
        if len(pipeline_output) != len(pipeline_input) or list(
            ...
            raise LxNeuralLemmatizerException(
                "Input and output lengths are not the same"
            )
        for sentence, output_sentence in zip(paragraph, pipeline_output):
            for token, output_token in zip(sentence, output_sentence):
                rule = output_token["entity"]
                token.lemma = lemma_edit.apply_lemma_rule(token.form, rule)
        return paragraph

    def lemmatize_sentence(self, sentence):
        return self.lemmatize_paragraph(lxcommon.LxParagraph(sentence))[0]
...
if __name__ == '__main__':
    ...
    lemmatizer = LxNeuralLemmatizer()
    for line in sys.stdin:
        sentence = tokenizer.tokenize_raw_sentence(line)
        lemmatizer.lemmatize_sentence(sentence)
        for token in sentence:
            print(token.form, token.lemma, sep="\t")
        print()
